...
std::optional<QImage> TGADecoder::decodeImage(const QString& imagePath) {
	...
	// Create stream
	FileStream imageStream{imagePath.toLocal8Bit().constData()};
	if (!imageStream) {
		return std::nullopt;
	}

	// Parse header
	auto header = imageStream.read<TGAHeader>();
	...
	std::vector<std::uint8_t> imageData(header.width * header.height * (header.bpp / 8));
	imageStream.seek_in(header.idLength + header.colorMapDepth, std::ios_base::cur);

	// Parse image data
	bool shouldFlip = false;
	if (header.dataTypeCode == 2) {
		// Uncompressed RGB
		imageStream.read(imageData.data(), imageData.size());
		shouldFlip = true;
		...
		} while (tempByteIndex < imageData.size());
	} else {
		return std::nullopt;
	}
